...
the L<Starman> webserver is used. Run C<cpan Starman> to install
it. The SQLite database is used to store the results, run C<cpan
DBD::SQLite> to install its driver and C<cpan DBI> to install the
database interface.
